  ...
  Timer.periodic(const Duration(seconds: 3), (timer) async {
    try {
      ...
      if (response.statusCode == 200) {
        // Sunucu düzeldiği için sunucu kronometresini sıfırla
        ...
        if (data != null && data['Temperature'] != null) {
          ...
          if (gelenTarih == sonLogTarihi) {
            ...
            if (ayniVeriSayaci >= 3) {
              sensorHataZamani ??= DateTime.now(); // Hatanın başladığı anı kaydet
              ...
              // 1 Dakika (60 sn) dolduysa ALARM çal!
              if (DateTime.now().difference(sensorHataZamani!).inSeconds >= 60) {
                if (!alarmCaldiSensor) {
                  bgBildirim.show(
                    11, '🚨 ALARM: SENSÖR KOPTU!', 'TAM 1 DAKİKADIR DONANIMDAN VERİ ALINAMIYOR!',
                    const NotificationDetails(android: AndroidNotificationDetails('alarm_kanali', 'Acil Alarmlar', importance: Importance.max, priority: Priority.high, color: Colors.red, enableVibration: true)),
                  );
                  alarmCaldiSensor = true;
                }
              } 
              // Henüz 1 dakika olmadıysa normal uyarı ver
              else if (!bildirimGonderildiSensor && !alarmCaldiSensor) {
                bgBildirim.show(
                  1, '📡 Sensör Bağlantısı Koptu', 'Donanımdan veri gelmiyor, durum izleniyor...',
                  const NotificationDetails(android: AndroidNotificationDetails('iot_kanali', 'Sistem Uyarıları', importance: Importance.max, priority: Priority.high)),
                );
                bildirimGonderildiSensor = true;
              }
            }
          } else {
            // Sensör verisi düzgün geliyorsa sensör kronometresini sıfırla
            ...
            if (service is AndroidServiceInstance) {
              service.setForegroundNotificationInfo(
                ...
                content: 'Anlık Sıcaklık: ${anlikSicaklik.toStringAsFixed(1)}°C',
              );
            }

            // 2. DURUM: KRİTİK SICAKLIK KONTROLÜ
            if (anlikSicaklik > 8.0) {
              sicaklikHataZamani ??= DateTime.now(); // Sıcaklığın eşiği geçtiği anı kaydet

              // 1 Dakika (60 sn) dolduysa ALARM çal!
              if (DateTime.now().difference(sicaklikHataZamani!).inSeconds >= 60) {
                if (!alarmCaldiSicaklik) {
                  bgBildirim.show(
                    12, '🚨 ALARM: KRİTİK SICAKLIK!', 'SICAKLIK 1 DAKİKADIR EŞİĞİN ÜZERİNDE (${anlikSicaklik.toStringAsFixed(1)}°C)!',
                    const NotificationDetails(android: AndroidNotificationDetails('alarm_kanali', 'Acil Alarmlar', importance: Importance.max, priority: Priority.high, color: Colors.red, enableVibration: true)),
                  );
              ...
              // Henüz 1 dakika olmadıysa normal uyarı ver
                ...
                bgBildirim.show(
                  2, '⚠️ Sıcaklık Uyarısı', 'Sıcaklık ${anlikSicaklik.toStringAsFixed(1)}°C oldu.',
                  const NotificationDetails(android: AndroidNotificationDetails('iot_kanali', 'Sistem Uyarıları', importance: Importance.max, priority: Priority.high, color: Colors.orange)),
                );
                bildirimGonderildiSicaklik = true;
              }
            } else {
              // Sıcaklık normale döndüyse kronometreyi sıfırla
        ...
        if (service is AndroidServiceInstance) {
          service.setForegroundNotificationInfo(title: 'IoT Cold Chain (Sunucu Hatası)', content: 'Merkezi sunucu cevap vermiyor.');
        }

        if (DateTime.now().difference(sunucuHataZamani!).inSeconds >= 60) {
          if (!alarmCaldiSunucu) {
            bgBildirim.show(
              13, '🚨 ALARM: SUNUCU ÇÖKTÜ!', 'TAM 1 DAKİKADIR SUNUCUDAN YANIT ALINAMIYOR!',
              const NotificationDetails(android: AndroidNotificationDetails('alarm_kanali', 'Acil Alarmlar', importance: Importance.max, priority: Priority.high, color: Colors.red, enableVibration: true)),
            );
          ...
          bgBildirim.show(
            3, '🔌 Sunucu Hatası', 'Merkezi sunucu cevap vermiyor.',
            const NotificationDetails(android: AndroidNotificationDetails('iot_kanali', 'Sistem Uyarıları', importance: Importance.max, priority: Priority.high)),
          );
      ...
      if (service is AndroidServiceInstance) {
        service.setForegroundNotificationInfo(title: 'IoT Cold Chain (Bağlantı Yok)', content: 'Sunucuya ulaşılamıyor.');
      }

      if (DateTime.now().difference(sunucuHataZamani!).inSeconds >= 60) {
        if (!alarmCaldiSunucu) {
          bgBildirim.show(
            13, '🚨 ALARM: BAĞLANTI KOPTU!', 'TAM 1 DAKİKADIR SİSTEME ULAŞILAMIYOR!',
            const NotificationDetails(android: AndroidNotificationDetails('alarm_kanali', 'Acil Alarmlar', importance: Importance.max, priority: Priority.high, color: Colors.red, enableVibration: true)),
          );
          alarmCaldiSunucu = true;
        }
        ...
        bgBildirim.show(
          3, '🔌 Sunucu Bağlantısı Koptu', 'FastAPI sunucusuna ulaşılamıyor.',
          const NotificationDetails(android: AndroidNotificationDetails('iot_kanali', 'Sistem Uyarıları', importance: Importance.max, priority: Priority.high)),
        );
        bildirimGonderildiSunucu = true;
      }
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Container(
                        decoration: BoxDecoration(
                          color: arkaPlanAktif ? Colors.red.withOpacity(0.1) : Colors.green.withOpacity(0.1),
                          shape: BoxShape.circle,
                          border: Border.all(
                            color: arkaPlanAktif ? Colors.red.withOpacity(0.3) : Colors.green.withOpacity(0.3)
                          ),
                        ),
                        child: IconButton(
                          icon: Icon(
                            arkaPlanAktif ? Icons.stop_rounded : Icons.play_arrow_rounded, 
                            size: 24
                          ),
                          color: arkaPlanAktif ? Colors.redAccent : Colors.greenAccent,
                          tooltip: arkaPlanAktif ? 'Takibi Durdur' : 'Takibi Başlat',
                          onPressed: () async {
                            final service = FlutterBackgroundService();
